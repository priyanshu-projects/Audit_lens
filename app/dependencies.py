...
app/dependencies.py
====================
Shared FastAPI dependencies — singleton model/data holders loaded once at startup.
Avoids reloading FinBERT, FAISS index, and processed claims on every request.
"""
...
def get_claims_df() -> pd.DataFrame:
    """Load claims.parquet once; return cached copy on subsequent calls."""
    global _claims_df
...
def invalidate_claims_cache() -> None:
    """Force reload on next get_claims_df() call (used after pipeline re-run)."""
    global _claims_df
    _claims_df = None


def get_documents() -> list:
    """Load documents.json once; return cached list."""
    global _documents
...
def get_rag_chain():
    """Build and cache RAG chain (loads FAISS index + Gemini LLM once)."""
    global _rag_chain
...
def get_verification_pipeline():
    """Build and cache VerificationPipeline (loads checkers once)."""
...
def get_finbert():
    """Load FinBERT model once and keep in memory."""
    global _finbert
    if _finbert is None:
        try:
