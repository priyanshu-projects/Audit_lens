...
app/api/verify.py
==================
POST /verify  — run consistency checks (L1/L2/L3) on claims on demand.
GET  /verify  — return cached verification_results.json if available.
"""
...
def _map_result(idx: int, r: dict) -> VerificationResultOut:
    return VerificationResultOut(
        claim_id=idx,
        claim_text=r.get("claim_text", r.get("text", "")),
        esg_label=r.get("esg_label", "MIXED"),
        risk_level=r.get("risk_level", r.get("aggregate_verdict", "LOW")),
        l1_status=r.get("l1_status", "UNKNOWN"),
        l2_status=r.get("l2_status", "UNKNOWN"),
        l3_status=r.get("l3_status", "UNKNOWN"),
        flags=r.get("flags", []),
        aggregate_verdict=r.get("aggregate_verdict", "LOW"),
...
async def get_verification_results() -> VerifyResponse:
    """Return cached verification results from the last pipeline run."""
...
async def run_verification(request: VerifyRequest) -> VerifyResponse:
    """
    Trigger on-demand verification (L1/L2/L3) for all or selected claim IDs.
    Results are written to data/processed/verification_results.json.
    """
    ...
    # Filter to requested claim IDs if specified
