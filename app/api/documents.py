...
GET  /documents/{doc_id} — get metadata for a single document
POST /documents/upload   — upload a new PDF/HTM for processing
...
def _build_doc_out(doc: dict, claims_df) -> DocumentOut:
    ...
    # Count claims for this document
...
async def list_documents() -> DocumentsListResponse:
    """List all documents that have been processed by the pipeline."""
    docs = get_documents()
    df   = get_claims_df()
...
async def get_document(doc_id: str) -> DocumentOut:
    ...
    df   = get_claims_df()
    match = next(
        (d for d in docs if d.get("doc_id") == doc_id or d.get("filename", "").startswith(doc_id)),
        None,
    )
    if not match:
...
async def upload_document(
    ...
    """
    Upload a new PDF or HTM filing.
    The file is saved to data/raw/ and extraction is queued as a background task.
    """
    ...
    # Queue extraction in background so upload returns immediately
    ...
    return UploadResponse(
        ...
        status="queued",
        message=f"File saved. Extraction pipeline queued for '{file.filename}'.",
    )
